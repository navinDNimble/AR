...
public class AugmentedFacesActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
    ...
    private final ObjectRenderer neckObject = new ObjectRenderer();
    ...
    private final float[] neckMatrix = new float[16];
...
//            augmentedFaceRenderer.setMaterialProperties(0.0f, 1.0f, 0.1f, 6.0f);
            ...
            neckObject.createOnGlThread(this, "models/model.obj", "models/neckless1.png");
            neckObject.setMaterialProperties(0.1f, 1.0f, 0.1f, 0.0f);
            neckObject.setBlendMode(ObjectRenderer.BlendMode.AlphaBlending);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        try {
            ...
            for (AugmentedFace face : faces) {
                ...
                // Render the neck object attached to the head.
                float[] neckMatrix = new float[16];
                face.getCenterPose().toMatrix(neckMatrix, 0);
                neckObject.updateModelMatrix(neckMatrix, scaleFactor);
                neckObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, DEFAULT_COLOR);
    ...
    private float[] estimateNeckPosition(Pose regionPose, Pose regionPose1, Pose regionPose2) {
        float[] neckPosition = new float[16];

        // Example: Simple averaging of the forehead landmarks.
        for (int i = 0; i < 16; i++) {
            neckPosition[i] = (regionPose.getTranslation()[i] +
                    regionPose1.getTranslation()[i] +
                    regionPose2.getTranslation()[i]) / 3.0f;
        }
